...
// Database handlers (Supports MySQL persistent storage on Hostinger / VPS, with local db.json fallback)
...
function getMySQLPool() {
  ...
  const port = process.env.MYSQL_PORT || process.env.DB_PORT ? parseInt(process.env.MYSQL_PORT || process.env.DB_PORT || "3306", 10) : 3306;
  ...
  if (host && user && database) {
    if (!dbPool) {
      ...
      console.log(`[PONPESQU Backend] Connected to MySQL database at ${host}:${port}/${database}`);
    }
    return dbPool;
  }
  return null;
    ...
    try {
      await pool.query(`
        CREATE TABLE IF NOT EXISTS app_state (
          id INT PRIMARY KEY,
          db_data LONGTEXT NOT NULL,
          updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP
        ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
      `);

      const [rows]: any = await pool.query("SELECT db_data FROM app_state WHERE id = 1");
      if (!rows || rows.length === 0) {
        let initialDb = DEFAULT_K_DB;
        if (fs.existsSync(DB_FILE)) {
          try {
            initialDb = JSON.parse(fs.readFileSync(DB_FILE, "utf-8"));
          } catch (e) {
            console.error("Failed to parse local db.json for initial MySQL seeding:", e);
          }
        }
        await pool.query("INSERT INTO app_state (id, db_data) VALUES (1, ?)", [JSON.stringify(initialDb)]);
        console.log("[PONPESQU Backend] Initialized and seeded MySQL app_state table.");
      }
    } catch (error) {
      console.error("[PONPESQU Backend] Error initializing MySQL table, will fallback to db.json:", error);
    }
  } else {
    console.log("[PONPESQU Backend] No MySQL environment variables found. Using local db.json storage.");
  }
}
...
async function loadDB(): Promise<any> {
  ...
  if (pool) {
    try {
      const [rows]: any = await pool.query("SELECT db_data FROM app_state WHERE id = 1");
      if (rows && rows.length > 0 && rows[0].db_data) {
        return JSON.parse(rows[0].db_data);
      }
    } catch (error) {
      console.error("[PONPESQU Backend] Error reading from MySQL, falling back to local db.json:", error);
    }
  }
  return loadDBLocal();
}
...
async function saveDB(data: any): Promise<void> {
  saveDBLocal(data); // Always write to local file as backup

  const pool = getMySQLPool();
  if (pool) {
    try {
      await pool.query(
        "INSERT INTO app_state (id, db_data) VALUES (1, ?) ON DUPLICATE KEY UPDATE db_data = VALUES(db_data)",
        [JSON.stringify(data)]
      );
    } catch (error) {
      console.error("[PONPESQU Backend] Error saving state to MySQL:", error);
    }
  }
}

...
export interface AbsensiSholat {
  ...
  sholat: string;
  id_santri: string;
  status: "TEPAT_WAKTU" | "MASBUQ" | "TIDAK_HADIR" | "IZIN" | "SAKIT";
  locked: boolean;
}
...
export interface IzinPulang {
  ...
  alasan: string;
  penjemput?: string;
  status: "MENUNGGU" | "DISETUJUI" | "DITOLAK" | "KEMBALI";
  dicatat_oleh: string;
...
export interface IzinMerokok {
  id_izin_merokok: string;
  id_santri: string;
  nama_santri: string;
  kelas: string;
  umur: number;
  ...
  tanggal_izin: string;
  masa_berlaku: string;
  ttd_orangtua: boolean;
  ttd_keamanan: boolean;
  ttd_kiai: boolean;
  status_izin: "AKTIF" | "MENUNGGU_TTD" | "DITOLAK" | "DICABUT";
  keterangan?: string;
  dicatat_oleh: string;
  tanggal_dicatat: string;
}

...
export function HomepageView({ db, onEnterLogin }: HomepageViewProps) {
  ...
  // Local toast message for sharing feedback
  const [shareToast, setShareToast] = useState<{ message: string; type: 'success' | 'info' } | null>(null);

  // News detail state (deep-linked from URL search query parameter 'newsId')
  const [selectedNewsId, setSelectedNewsId] = useState<string | null>(() => {
    try {
      const params = new URLSearchParams(window.location.search);
      const qId = params.get('newsId');
      if (qId) {
        // Verify this news actually exists in the db
        const exists = (db.homepage?.news || []).some((n: any) => n.id === qId) ||
                       ((db.homepage || {}).news || []).some((n: any) => n.id === qId);
        if (exists) return qId;
      }
    } catch (e) {
      console.error("Error reading initial newsId parameter:", e);
    }
    return null;
  });
  ...
  // Synchronize browser URL query parameter with selectedNewsId
  useEffect(() => {
    try {
      ...
      console.error("Error synchronizing URL parameter:", e);
    }
  }, [selectedNewsId]);
  ...
  const showShareToast = (message: string, type: 'success' | 'info' = 'success') => {
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 sm:py-12 animate-fadeIn bg-[#f1f6f4]">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
            ...
            <div className="lg:col-span-8 bg-white border border-[#e5eeea] rounded-3xl p-5 sm:p-8 shadow-sm space-y-6">
              ...
              {/* Share Block and Back button */}
              <div className="border-t border-[#e1eae6] pt-6 mt-8 space-y-6">
                
                {/* News Share Options */}
                <div className="bg-[#f8fbf9] border border-[#e1eae6] rounded-2xl p-4 sm:p-5">
                  <h3 className="text-xs sm:text-sm font-bold text-[#064e3b] mb-3.5 flex items-center gap-2">
                    <LucideIcon name="share-2" className="w-4.5 h-4.5 text-[#10b981]" />
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                    ...
                    <button
                      ...
                      onClick={() => {
                        ...
                        const text = `Baca berita terbaru dari ${db.settings.nama_pesantren || "Pesantren kami"}: "${selectedNews.title}"\n\nSelengkapnya di: ${shareUrl}`;
                        window.open(`https://api.whatsapp.com/send?text=${encodeURIComponent(text)}`, '_blank');
                        showShareToast("Membuka WhatsApp untuk membagikan...", "success");
                      }}
                      className="flex items-center justify-center gap-2 px-4 py-2.5 bg-[#25D366]/10 hover:bg-[#25D366]/20 text-[#128C7E] font-bold text-xs sm:text-sm rounded-xl border border-[#25D366]/30 transition-all shadow-sm active:scale-95 cursor-pointer"
                    >
                      <svg className="w-4 h-4 fill-current" viewBox="0 0 24 24">
                        <path d="M17.472 14.382c-.022-.008-.115-.062-.247-.115s-.713-.356-.822-.395-.197-.062-.28.063-.319.396-.39.475-.145.09-.223.05a10.155 10.155 0 0 1-2.91-1.794 10.2 10.2 0 0 1-2.017-2.512c-.078-.135-.008-.21.06-.276.06-.06.135-.145.203-.223a.99.99 0 0 0 .145-.24c.039-.074.02-.137-.01-.197-.03-.06-.28-.68-.385-.94-.105-.252-.21-.22-.28-.22-.073-.005-.158-.005-.243-.005s-.223.033-.34.158c-.115.127-.442.433-.442 1.057 0 .624.455 1.228.519 1.312.063.085.897 1.37 2.175 1.922.304.13.542.207.727.266.306.096.584.082.802.05.243-.037.713-.292.812-.562.1-.27.1-.5-.03-.55-.022-.01-.115-.062-.247-.115M12 2C6.477 2 2 6.477 2 12c0 1.89.525 3.66 1.438 5.168L2.05 22l4.982-1.306A9.925 9.925 0 0 0 12 22c5.523 0 10-4.477 10-10S17.523 2 12 2zm0 18c-1.634 0-3.156-.419-4.48-1.155l-.32-.19-2.97.778.792-2.898-.208-.33A7.95 7.95 0 0 1 4 12c0-4.41 3.59-8 8-8s8 3.59 8 8-3.59 8-8 8z"/>
                      </svg>
                      <span>WhatsApp</span>
                    </button>
                    ...
                    <button
                      ...
                      onClick={() => {
                        ...
                        showShareToast("Membuka Facebook untuk membagikan...", "success");
                      }}
                      className="flex items-center justify-center gap-2 px-4 py-2.5 bg-[#1877F2]/10 hover:bg-[#1877F2]/20 text-[#1877F2] font-bold text-xs sm:text-sm rounded-xl border border-[#1877F2]/30 transition-all shadow-sm active:scale-95 cursor-pointer"
                    >
                      <svg className="w-4 h-4 fill-current" viewBox="0 0 24 24">
                    ...
                    {/* Instagram Help Copier */}
                    <button
                      type="button"
                      onClick={() => {
                        const shareUrl = `${window.location.origin}${window.location.pathname}?newsId=${selectedNews.id}`;
                        navigator.clipboard.writeText(shareUrl);
                        showShareToast("Tautan disalin! Membuka Instagram...", "success");
                        setTimeout(() => {
                          window.open('https://www.instagram.com/', '_blank');
                        }, 800);
                      }}
                      className="flex items-center justify-center gap-2 px-4 py-2.5 bg-[#E1306C]/10 hover:bg-[#E1306C]/20 text-[#C13584] font-bold text-xs sm:text-sm rounded-xl border border-[#E1306C]/30 transition-all shadow-sm active:scale-95 cursor-pointer"
                    ...
                    <button
                      ...
                      onClick={() => {
                        const shareUrl = `${window.location.origin}${window.location.pathname}?newsId=${selectedNews.id}`;
                        navigator.clipboard.writeText(shareUrl);
                        showShareToast("✓ Tautan berita berhasil disalin ke papan klip!", "success");
                      }}
                      className="flex items-center justify-center gap-2 px-4 py-2.5 bg-white hover:bg-[#f1f6f4] text-[#064e3b] font-bold text-xs sm:text-sm rounded-xl border border-[#d1ded9] transition-all shadow-sm active:scale-95 cursor-pointer"
                    >
                      <LucideIcon name="copy" className="w-4 h-4 text-[#10b981]" />
                      <span>Salin Tautan</span>
                    </button>

                  </div>

                  {/* Dynamic Toast Feedback Overlay style inside share panel */}
                  {shareToast && (
                    <div className="mt-3.5 p-3 bg-emerald-950 text-white rounded-xl border border-emerald-800 text-[11px] font-semibold flex items-center gap-2 animate-fadeIn">
                ...
                <div className="flex justify-start">
                  <button 
                    type="button"
                    onClick={() => setSelectedNewsId(null)}
                    className="w-full sm:w-auto px-5 py-2.5 bg-[#f1f6f4] hover:bg-[#e6f2ee] text-[#064e3b] font-bold rounded-xl text-xs sm:text-sm transition-colors flex items-center justify-center gap-2 border border-[#d1ded9] cursor-pointer"
                  >
              ...
              >
                <div className="aspect-[16/10] bg-emerald-950 overflow-hidden relative">
                  ...
                  <button
                    ...
                    onClick={(e) => {
                      e.stopPropagation();
                      const cardShareUrl = `${window.location.origin}${window.location.pathname}?newsId=${n.id}`;
                      navigator.clipboard.writeText(cardShareUrl);
                      showShareToast("✓ Tautan berita berhasil disalin!", "success");
                    }}
                    className="absolute top-3 right-3 bg-white/90 hover:bg-white text-[#064e3b] hover:text-[#10b981] p-2 rounded-full border border-[#10b981]/30 shadow-md transition-all cursor-pointer z-10"
                    title="Salin Tautan Berita"
                  >
                    <LucideIcon name="share-2" className="w-3.5 h-3.5 text-[#10b981]" />
                  </button>
                </div>

...
export function PengajarPanel({
  ...
  // Real-time live clock for scan time (HH:MM:SS) automatically synchronized with device
  const [liveScanTime, setLiveScanTime] = useState<string>(() => {
    const d = new Date();
  ...
  useEffect(() => {
    const updateTimer = () => {
      const d = new Date();
      setLiveScanTime(`${d.getHours().toString().padStart(2, '0')}:${d.getMinutes().toString().padStart(2, '0')}:${d.getSeconds().toString().padStart(2, '0')}`);
    };
    updateTimer();
  ...
  const handlePrayerScan = (santri: Santri) => {
    ...
    const timeStr = getNowTimeStr();
    const currentHHMM = timeStr.slice(0, 5);
    ...
    syncDbState(updatedDb).then(() => {
      showToast(`Scan Sholat Berhasil: ${santri.nama_santri} dicatat ${computedStatus.replace('_', ' ')} (Pukul ${timeStr}).`, "success");
    });
  };
  ...
  const handlePrayerStatusChange = (santriId: string, newStatus: "TEPAT_WAKTU" | "MASBUQ" | "TIDAK_HADIR") => {
    ...
    if (newStatus !== "TIDAK_HADIR" && !scannedPrayerTimes[santriId]) {
      setScannedPrayerTimes(prev => ({ ...prev, [santriId]: getNowTimeStr() }));
    }
  ...
  const handleAddIzin = (e: React.FormEvent) => {
    ...
    const dates = getDatesInRange(newIzinMulai, newIzinSelesai);
    const updatedAbsensiKelas = [...absensiKelasList];
    const updatedAbsensiSholat = [...absensiSholatList];
    const attendanceStatus = newIzinTipe; // "IZIN" or "SAKIT"

    const defaultSholatRules = [
      ...
      { id_sholat: 'SLT-004', nama: 'Sholat MAGHRIB', tipe: 'WAJIB', waktu: '18:00', toleransi: 15 },
      { id_sholat: 'SLT-005', nama: 'Sholat ISYA', tipe: 'WAJIB', waktu: '19:15', toleransi: 15 }
    ];
    const sholatList = sholatRules.length > 0 ? sholatRules : defaultSholatRules;

    dates.forEach(dStr => {
      // 1. Sync Absensi Kelas (KBM)
      const existingKelasIdx = updatedAbsensiKelas.findIndex(
        a => a.tanggal === dStr && a.id_santri === newIzinSantriId
      );
      if (existingKelasIdx > -1) {
        updatedAbsensiKelas[existingKelasIdx] = {
          ...updatedAbsensiKelas[existingKelasIdx],
      ...
      // 2. Sync Absensi Sholat Berjamaah
      sholatList.forEach(rule => {
        const existingSholatIdx = updatedAbsensiSholat.findIndex(
          a => a.tanggal === dStr && a.sholat === rule.id_sholat && a.id_santri === newIzinSantriId
        );
        if (existingSholatIdx > -1) {
          updatedAbsensiSholat[existingSholatIdx] = {
            ...updatedAbsensiSholat[existingSholatIdx],
            status: attendanceStatus
          };
        } else {
    ...
    syncDbState(updatedDb).then(() => {
      showToast(`Izin ${newIzinTipe} ${santriObj.nama_santri} berhasil terdaftar & otomatis terintegrasi ke Absensi KBM & Sholat Berjamaah!`, "success");
  ...
  const handleApproveIzin = (izin: Perizinan) => {
    showConfirm(`Setujui pengajuan izin ${izin.tipe} untuk santri ini? Absensi KBM & Sholat Berjamaah tanggal terkait otomatis terisi.`, (yes) => {
      if (yes) {
        ...
        const updatedAbsensiKelas = [...absensiKelasList];
        const updatedAbsensiSholat = [...absensiSholatList];
        const dates = getDatesInRange(izin.tanggal_mulai, izin.tanggal_selesai);
        const attendanceStatus = izin.tipe;
        ...
        if (santriObj) {
          dates.forEach(dStr => {
            // Absensi KBM
            const existingKelasIdx = updatedAbsensiKelas.findIndex(
              a => a.tanggal === dStr && a.id_santri === izin.id_santri
            );
            if (existingKelasIdx > -1) {
              updatedAbsensiKelas[existingKelasIdx] = {
                ...updatedAbsensiKelas[existingKelasIdx],
            ...
            sholatList.forEach(rule => {
              const existingSholatIdx = updatedAbsensiSholat.findIndex(
                a => a.tanggal === dStr && a.sholat === rule.id_sholat && a.id_santri === izin.id_santri
              );
              if (existingSholatIdx > -1) {
                updatedAbsensiSholat[existingSholatIdx] = {
                  ...updatedAbsensiSholat[existingSholatIdx],
        ...
        syncDbState(updatedDb).then(() => {
          showToast("Izin telah disetujui & otomatis terintegrasi ke sistem presensi KBM dan Sholat Berjamaah.", "success");
        });
      }
    });
  };
          ...
          <div className="glass-card p-5 rounded-2xl border border-emerald-500/15">
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 pb-4 border-b border-emerald-950/50">
              ...
              <div className="flex flex-wrap items-center gap-3 w-full md:w-auto">
                ...
                <div className="flex items-center gap-2 bg-emerald-950/90 px-3 py-1.5 rounded-xl border border-amber-500/40 text-xs shadow-inner">
                  <LucideIcon name="clock" className="w-3.5 h-3.5 text-amber-400 animate-pulse" />
                  <span className="text-gray-300 text-[10px] font-bold uppercase tracking-wider">Jam Real-time HP/Laptop:</span>
                  <span className="text-amber-400 font-mono font-extrabold text-xs">{liveScanTime}</span>
                        ...
                        return (
                          ...
                          >
                            ...
                            <span className={`text-[10px] font-mono shrink-0 font-bold ${
                              ...
                              {status !== "BELUM_ABSEN" && status !== "TIDAK_HADIR" ? `[${timeRecorded || liveScanTime.slice(0, 5)}]` : '[BELUM]'}
                            </span>
                          </div>
                        );
                      })}

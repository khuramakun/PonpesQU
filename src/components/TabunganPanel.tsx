...
export function TabunganPanel({
  ...
  // Sub-tab selection for scanned student profile (Tabungan vs Belanja)
  const [subTab, setSubTab] = useState<'TABUNGAN' | 'BELANJA'>('TABUNGAN');
  ...
  if (activeTab === 'tabungan-loket') {
    ...
    const selectedSantriMarketHistory = selectedSantri
      ? (db.transaksi_market || []).filter(t => t.id_santri === selectedSantri.id_santri)
      : [];
          ...
          <div className="lg:col-span-4 glass-card p-5 rounded-2xl flex flex-col gap-4">
            ...
            {!selectedSantri ? (
              ...
              <div className="flex flex-col gap-4">
                {/* Profile card */}
                <div className="flex items-center gap-3.5 bg-emerald-950/40 p-3 rounded-2xl border border-emerald-900/40 shadow-sm">
                  <img 
                  ...
                  <div className="flex-1 min-w-0">
                    <h5 className="text-sm font-bold text-white uppercase tracking-wide truncate">{selectedSantri.nama_santri}</h5>
                    <p className="text-xs text-amber-400 font-semibold mt-0.5">
                      {selectedSantri.kelas}
                      {selectedSantri.alamat && ` • 📍 ${selectedSantri.alamat}`}
                    </p>
                    <span className="text-[10px] bg-emerald-500/10 text-emerald-400 border border-emerald-500/20 px-2 py-0.5 rounded-md font-mono inline-block mt-2">ID: {selectedSantri.barcode}</span>
                  </div>
                </div>
                ...
                {/* HISTORIC TRANSACTIONS TABS & LIST */}
                <div className="flex flex-col gap-1.5 mt-1">
                  <div className="flex justify-between items-center border-b border-emerald-950/40 pb-1">
                    <span className="text-[10px] font-bold text-emerald-500/80 uppercase tracking-wide">Aktivitas Kartu Santri</span>
                    <div className="flex gap-2">
                      <button
                        type="button"
                        onClick={() => setSubTab('TABUNGAN')}
                        className={`text-[9px] font-bold px-2 py-0.5 rounded transition-all ${subTab === 'TABUNGAN' ? 'bg-amber-500/20 text-amber-400 border border-amber-500/30' : 'text-emerald-500/50 hover:text-emerald-500/80'}`}
                      >
                        Tabungan
                      </button>
                      <button
                        type="button"
                        onClick={() => setSubTab('BELANJA')}
                        className={`text-[9px] font-bold px-2 py-0.5 rounded transition-all ${subTab === 'BELANJA' ? 'bg-amber-500/20 text-amber-400 border border-amber-500/30' : 'text-emerald-500/50 hover:text-emerald-500/80'}`}
                      >
                        Belanja {selectedSantriMarketHistory.length > 0 && `(${selectedSantriMarketHistory.length})`}
                      </button>
                    </div>
                  </div>
                  ...
                  {subTab === 'TABUNGAN' ? (
                    <div className="max-h-[140px] overflow-y-auto no-scrollbar border border-emerald-950/60 rounded-xl bg-emerald-950/20 p-1.5 flex flex-col gap-1.5">
                      {selectedSantriHistory.length === 0 ? (
                        <p className="text-[9px] text-emerald-500/40 text-center py-6">Belum ada riwayat transaksi tabungan.</p>
                      ) : (
                    ...
                    <div className="max-h-[140px] overflow-y-auto no-scrollbar border border-emerald-950/60 rounded-xl bg-emerald-950/20 p-1.5 flex flex-col gap-1.5">
                      {selectedSantriMarketHistory.length === 0 ? (
                        <p className="text-[9px] text-emerald-500/40 text-center py-6">Belum ada riwayat belanja market.</p>
                      ) : (
                        selectedSantriMarketHistory.slice().reverse().map(tx => {
                          const itemsList = tx.items.map(it => `${it.nama_produk} (x${it.qty})`).join(', ');
                          const isCashless = tx.metode_pembayaran === 'TABUNGAN';
                          return (
                            <div key={tx.id_transaksi} className="p-2 bg-emerald-950/40 rounded-lg border border-emerald-900/10 flex flex-col gap-1 text-[9px]">
                              <div className="flex justify-between items-start">
                                <div className="truncate">
                                  <span className="font-bold text-gray-200 block truncate">
                                    🛒 Belanja {isCashless ? 'Cashless' : 'Tunai'}
                                  </span>
                                  <span className="text-[8px] text-emerald-500/40 block font-mono">{tx.tanggal}</span>
                                </div>
                                <span className="font-mono font-extrabold text-red-400 shrink-0 pl-2">
                                  -{formatRupiah(tx.total)}
                                </span>
                              </div>
                              {itemsList && (
                                <p className="text-[8px] text-emerald-400/60 italic line-clamp-1 truncate leading-relaxed">
                                  {itemsList}
                                </p>
                              )}
                            </div>
  ...
  if (activeTab === 'tabungan-riwayat') {
    // Combine savings transactions (deposits & manual withdrawals) and cooperative shopping transactions
    const combinedLedger: Array<{
      id_transaksi: string;
      tanggal: string;
      id_santri: string;
      nama_santri: string;
      kelas: string;
      tipe: "DEPOSIT" | "PENARIKAN" | "BELANJA_TABUNGAN" | "BELANJA_TUNAI";
      nominal: number;
      keterangan: string;
      itemsList?: string;
    }> = [];

    // 1. Add saving transactions
    (db.transaksi_tabungan || []).forEach(t => {
      combinedLedger.push({
    ...
    // 2. Add market cooperative transactions
    (db.transaksi_market || []).forEach(m => {
      const s = db.santri.find(x => x.id_santri === m.id_santri);
      const sClass = s ? s.kelas : "Umum/Luar";
      const itemsStr = m.items ? m.items.map(it => `${it.nama_produk} (x${it.qty})`).join(', ') : '';
      combinedLedger.push({
        ...
        kelas: sClass,
        tipe: m.metode_pembayaran === "TABUNGAN" ? "BELANJA_TABUNGAN" : "BELANJA_TUNAI",
        nominal: m.total,
        keterangan: `Belanja Kantin/Koperasi: ${itemsStr || 'Produk Market'}`,
        itemsList: itemsStr
      });
    });

    // Sort by date descending (newest first)
    combinedLedger.sort((a, b) => b.tanggal.localeCompare(a.tanggal));

    const q = mutationSearch.toLowerCase();
    const filteredMutations = combinedLedger.filter(t => {
      const matchesSearch = t.nama_santri.toLowerCase().includes(q) || 
                            t.keterangan.toLowerCase().includes(q) ||
                            (t.itemsList && t.itemsList.toLowerCase().includes(q)) ||
                            t.id_transaksi.toLowerCase().includes(q);
      
      let matchesType = true;
      if (mutationTypeFilter !== "ALL") {
        if (mutationTypeFilter === "BELANJA") {
          matchesType = t.tipe === "BELANJA_TABUNGAN" || t.tipe === "BELANJA_TUNAI";
        } else {
          matchesType = t.tipe === mutationTypeFilter;
        }
      }
    ...
    return (
      <section id="tab-tabungan-riwayat" className="tab-content flex flex-col gap-6">
        <div className="glass-card p-5 rounded-2xl">
          ...
          <p className="text-xs text-emerald-500/70">Daftar rekam histori lengkap setoran masuk, penarikan tunai loket, belanja cashless santri di kantin, dan rincian mutasi pembukuan saku.</p>
        </div>
        ...
        <div className="glass-card p-4 rounded-2xl flex flex-col sm:flex-row gap-3">
          <div className="relative flex-grow">
            ...
            <input 
              ...
              placeholder="Cari berdasarkan nama santri, ID transaksi, keterangan, atau barang..." 
          ...
          >
            ...
            <option value="BELANJA">Semua Riwayat Belanja (Kantin/Koperasi)</option>
            <option value="BELANJA_TABUNGAN">Hanya Belanja Cashless (Tabungan)</option>
            <option value="BELANJA_TUNAI">Hanya Belanja Tunai (Tunai)</option>
          </select>
        </div>
          ...
          <table className="w-full text-left border-collapse text-xs">
            <thead>
              <tr className="border-b border-emerald-950 text-emerald-500/70 uppercase text-[9px] tracking-wider">
                ...
                <th className="py-3 text-center">Struk / Bukti</th>
              </tr>
            </thead>
                ...
                filteredMutations.map((t, idx) => {
                  ...
                  return (
                    <tr key={idx} className="hover:bg-emerald-950/10 border-b border-emerald-950/10 transition-colors">
                      ...
                      <td className="py-3 text-emerald-500/80 max-w-xs truncate" title={t.keterangan}>
                        <span className="italic">"{t.keterangan}"</span>
                      </td>
                      <td className="py-3 text-center">
                        <span className={`px-2.5 py-0.5 rounded-full text-[8px] font-extrabold tracking-wider ${badgeStyle}`}>
                          {label}
                        </span>
                      </td>
                      ...
                      <td className="py-3 text-center">
                        <button 
                          ...
                          className="p-1.5 bg-emerald-900/40 hover:bg-emerald-900 border border-emerald-800 text-amber-400 rounded-lg transition-all"
        ...
        {isSlipOpen && previewTx && (
          <dialog open className="backdrop:bg-[#02110e]/80 fixed inset-0 z-50 bg-transparent focus:outline-none p-4 w-full max-w-sm flex items-center justify-center animate-fade-in">
            <div className="bg-amber-50 text-emerald-950 p-6 rounded-2xl shadow-2xl relative font-mono text-xs flex flex-col gap-4 border border-amber-200 w-full">
              <div id="print-area-saving-slip-wrapper" className="w-full">
                <div id="print-area-saving-slip" className="p-3 bg-white text-black rounded border border-gray-300 w-full">
                  <div className="text-center border-b border-dashed border-gray-400 pb-2 mb-2">
                    ...
                    {previewTx.tipe.startsWith("BELANJA") ? (
                      <>
                        <p className="text-[8px] text-gray-500 leading-none mt-1">KANTIN / KOPERASI SMART MARKET</p>
                        <span className="text-[9px] bg-amber-100 text-amber-900 px-2 py-0.5 rounded font-bold uppercase tracking-wider inline-block mt-2">
                          {previewTx.tipe === "BELANJA_TABUNGAN" ? "STRUK BELANJA CASHLESS" : "STRUK BELANJA TUNAI"}
                        </span>
                      </>
                    ) : (
                      <>
                  ...
                  {/* Item List for Market Transactions */}
                  {previewTx.tipe.startsWith("BELANJA") && (
                    <div className="border-t border-dashed border-gray-400 pt-2 space-y-1 mb-2">
                      <span className="text-[8px] text-gray-400 block mb-1">PRODUK YANG DIBELI:</span>
                      {previewTx.itemsList ? (
                        previewTx.itemsList.split(', ').map((item: string, idx: number) => (
                          <div key={idx} className="flex justify-between text-[9px] text-gray-700">
                            <span>• {item}</span>
                          </div>
                        ))
                      ) : (
                        <p className="text-[9px] text-gray-500 italic">Rincian produk tidak tersedia</p>
                      )}
                    </div>
                  )}
                  ...
                  <div className="border-t border-b border-dashed border-gray-400 py-2 my-2 text-center">
                    <span className="text-[9px] text-gray-500 block uppercase tracking-wider">
                      {previewTx.tipe === "DEPOSIT" ? "Jumlah Setoran" : 
                       previewTx.tipe === "PENARIKAN" ? "Jumlah Penarikan" : "Total Belanja"}
                    </span>
                    <span className="text-sm font-extrabold text-emerald-950 block">{formatRupiah(previewTx.nominal)}</span>
                  </div>

                  {previewTx.tipe !== "BELANJA_TUNAI" && (
                    <div className="flex justify-between text-[10px] mt-2">
                      <span>{previewTx.tipe.startsWith("BELANJA") ? "Sisa Saldo Saku:" : "Saldo Akhir Saku:"}</span>
                      <span className="font-bold text-emerald-800">{formatRupiah(previewBalance)}</span>
                    </div>
                  )}
                  ...
                  <div className="flex justify-between items-center text-[9px] mt-4 pt-2 border-t border-dashed border-gray-400">
                    ...
                    <div className="text-center w-24 font-mono text-[6px] text-gray-500">
                      {previewTx.tipe.startsWith("BELANJA") ? "*Syukron Jazakumullah Khairan*" : "*Simpan bukti ini sebagai tanda terima sah*"}
                    </div>
                    <div className="text-center w-20">
                      <span className="block">{previewTx.tipe.startsWith("BELANJA") ? "Kasir" : "Bendahara"}</span>
                      <div className="h-6"></div>
                      <span className="border-t border-black pt-1 block font-bold text-[8px]">
                        {previewTx.tipe.startsWith("BELANJA") ? "Ummi Halimah" : "Ustadz Jafar"}
                      </span>
                    </div>
                  </div>
                </div>

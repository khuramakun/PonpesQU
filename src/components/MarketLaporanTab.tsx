...
export function MarketLaporanTab({
  ...
  const dailyChartData = React.useMemo(() => {
    const map: Record<string, { tanggal: string; omset: number; profit: number; count: number }> = {};
    const sortedTx = [...filteredTransactions].sort((a, b) => a.tanggal.localeCompare(b.tanggal));

    sortedTx.forEach(tx => {
      const d = tx.tanggal.slice(0, 10);
      if (!map[d]) {
        map[d] = { tanggal: d, omset: 0, profit: 0, count: 0 };
      }
      map[d].omset += tx.total || 0;

      let pVal = tx.profit;
      if (pVal === undefined) {
        pVal = tx.items.reduce((pSum, item) => {
          const p = products.find(x => x.id_produk === item.id_produk);
          if (p) return pSum + ((p.harga_jual - p.harga_beli) * item.qty);
          return pSum;
        }, 0);
  ...
  const topProductsData = React.useMemo(() => {
    ...
    filteredTransactions.forEach(tx => {
      tx.items.forEach(it => {
        if (!it.id_produk || it.id_produk === 'MUTASI') return;
        const pName = it.nama_produk || it.id_produk;
        if (!map[pName]) {
          map[pName] = { name: pName, qty: 0, total: 0 };
        }
        map[pName].qty += it.qty || 0;
        map[pName].total += (it.harga_jual || 0) * (it.qty || 0);
      });
    });
  ...
  return (
    <div className="flex flex-col gap-6 w-full">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 w-full">
        ...
        <div className="glass-card p-4 rounded-2xl">
          <span className="text-[10px] text-emerald-500/70 uppercase tracking-wider block font-bold">Laba Bersih</span>
          <span className="text-lg font-extrabold text-emerald-400 block mt-1">{formatRupiah(totalProfit)}</span>
          <span className="text-[10px] text-emerald-500/40">Keuntungan (Penjualan - Modal)</span>
        </div>
        <div className="glass-card p-4 rounded-2xl">
      ...
      {/* Visual Sales Charts Section */}
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 w-full">
        {/* Main Daily Trend Chart */}
        <div className="lg:col-span-8 glass-card p-5 rounded-2xl border border-emerald-500/20 bg-emerald-950/20 flex flex-col justify-between">
          <div className="flex items-center justify-between border-b border-emerald-900/40 pb-3 mb-4 flex-wrap gap-2">
            <div className="flex items-center gap-2">
              <span className="p-1.5 bg-amber-500/10 rounded-lg text-amber-400 border border-amber-500/20">
                <LucideIcon name="bar-chart-3" className="w-4 h-4" />
              </span>
              <div>
                <h3 className="text-xs font-bold text-gray-100 uppercase tracking-wide">Grafik Tren Omset & Laba Penjualan Harian</h3>
                <p className="text-[10px] text-emerald-400/80">Grafik visual dinamika transaksi belanja santri dan profitabilitas market</p>
              </div>
            </div>
            <span className="text-[10px] font-mono text-emerald-400 bg-emerald-950 px-2 py-1 rounded border border-emerald-800">
              {dailyChartData.length} Hari Terdata
            </span>
          </div>
            ...
            {dailyChartData.length > 0 ? (
              <ResponsiveContainer width="100%" height="100%">
                <AreaChart data={dailyChartData} margin={{ top: 10, right: 10, left: 0, bottom: 0 }}>
                  ...
                  <CartesianGrid strokeDasharray="3 3" stroke="#064e3b" opacity={0.4} />
                  <XAxis dataKey="tanggal" stroke="#6ee7b7" fontSize={10} tickLine={false} />
                  <YAxis stroke="#6ee7b7" fontSize={10} tickFormatter={(val) => `Rp${(val/1000).toFixed(0)}k`} />
                  <Tooltip
                    contentStyle={{ backgroundColor: '#021814', borderColor: '#10b981', borderRadius: '12px', fontSize: '11px', color: '#fff' }}
                    formatter={(value: any, name: any) => [formatRupiah(Number(value)), name === 'omset' ? 'Omset Total' : 'Laba Bersih']}
                    labelFormatter={(label) => `Tanggal: ${label}`}
                  />
                  <Legend wrapperStyle={{ fontSize: '11px', paddingTop: '8px' }} />
                  <Area type="monotone" dataKey="omset" name="Omset Total" stroke="#f59e0b" strokeWidth={2} fillOpacity={1} fill="url(#omsetGradient)" />
                  <Area type="monotone" dataKey="profit" name="Laba Bersih" stroke="#10b981" strokeWidth={2} fillOpacity={1} fill="url(#profitGradient)" />
                </AreaChart>
              </ResponsiveContainer>
            ) : (
              <div className="h-full flex items-center justify-center text-gray-500 italic text-xs">
                Belum ada data grafik transaksi pada periode yang dipilih.
              </div>
            )}
          </div>
        </div>
        ...
        <div className="lg:col-span-4 glass-card p-5 rounded-2xl border border-emerald-500/20 bg-emerald-950/20 flex flex-col justify-between">
          <div className="border-b border-emerald-900/40 pb-3 mb-4">
            <h3 className="text-xs font-bold text-gray-100 uppercase tracking-wide flex items-center gap-2">
              <LucideIcon name="shopping-bag" className="w-4 h-4 text-emerald-400" />
              <span>Top 5 Produk Terlaris</span>
            </h3>
            <p className="text-[10px] text-emerald-400/80 mt-0.5">Produk paling diminati berdasarkan Qty terjual</p>
          </div>
            ...
            {topProductsData.length > 0 ? (
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={topProductsData} layout="vertical" margin={{ top: 5, right: 10, left: 10, bottom: 5 }}>
                  ...
                  <XAxis type="number" stroke="#6ee7b7" fontSize={10} />
                  <YAxis dataKey="name" type="category" stroke="#6ee7b7" fontSize={9} width={90} tickFormatter={(val) => val.length > 12 ? val.slice(0, 12) + '...' : val} />
                  <Tooltip
                    contentStyle={{ backgroundColor: '#021814', borderColor: '#f59e0b', borderRadius: '12px', fontSize: '11px', color: '#fff' }}
                    formatter={(value: any) => [`${value} pcs`, 'Qty Terjual']}
                  />
                  <Bar dataKey="qty" name="Qty Terjual" fill="#10b981" radius={[0, 8, 8, 0]} />
                </BarChart>
              </ResponsiveContainer>
            ) : (
              <div className="h-full flex items-center justify-center text-gray-500 italic text-xs">

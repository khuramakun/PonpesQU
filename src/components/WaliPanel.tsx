...
import { K_DB, Santri, Keluhan, IzinPulang } from '../types';
...
export function WaliPanel({
  ...
  const [izinKeluarDate, setIzinKeluarDate] = useState(todayStr);
  const [izinKembaliDate, setIzinKembaliDate] = useState(todayStr);
  const [izinAlasan, setIzinAlasan] = useState("");
  const [izinPenjemput, setIzinPenjemput] = useState("");
  const [isSubmittingIzin, setIsSubmittingIzin] = useState(false);

  // Izin Pulang filter state
  const [izinFilterStatus, setIzinFilterStatus] = useState<"ALL" | "MENUNGGU" | "DISETUJUI" | "TERLAMBAT" | "KEMBALI" | "DITOLAK">("ALL");
  const [izinSearchText, setIzinSearchText] = useState("");
  ...
  // Filter child home leave history (Izin Pulang)
  const childIzinPulangList = (db.izin_pulang || []).filter(i => 
    i.id_santri === child.id_santri || 
    (i.nama_santri && i.nama_santri.toLowerCase() === child.nama_santri.toLowerCase())
  );

  const handleApplyIzinPulang = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!izinKeluarDate || !izinKembaliDate || !izinAlasan.trim()) {
      showToast("Harap isi tanggal keluar, tanggal kembali, dan alasan izin pulang!", "error");
      return;
    }
    if (izinKembaliDate < izinKeluarDate) {
      showToast("Tanggal kembali tidak boleh lebih awal dari tanggal keluar!", "error");
      return;
    }

    setIsSubmittingIzin(true);
    const newIzin: IzinPulang = {
      id_izin_pulang: `IZN-${Date.now().toString().slice(-4)}`,
      id_santri: child.id_santri,
      nama_santri: child.nama_santri,
      kelas: child.kelas,
      tanggal_keluar: izinKeluarDate,
      tanggal_kembali: izinKembaliDate,
      alasan: izinAlasan.trim(),
      penjemput: izinPenjemput.trim() || child.nama_wali || "Orang Tua / Wali",
      status: "MENUNGGU",
      dicatat_oleh: "Santri/Wali (Pengajuan Portal)",
      tanggal_dicatat: todayStr
    };

    const updatedDb: K_DB = {
      ...db,
      izin_pulang: [newIzin, ...(db.izin_pulang || [])]
    };
    ...
    showToast(`✓ Permohonan izin pulang untuk ${child.nama_santri} berhasil diajukan! Menunggu persetujuan Pos Keamanan.`, "success");
  };

  const handleCancelIzin = (idIzin: string) => {
    showConfirm("Batalkan permohonan izin pulang ini?", async (yes) => {
      if (yes) {
        const updatedList = (db.izin_pulang || []).filter(i => i.id_izin_pulang !== idIzin);
        await syncDbState({ ...db, izin_pulang: updatedList });
        showToast("Permohonan izin pulang berhasil dibatalkan.", "info");
      }
    });
  };
  ...
  return (
    <div className="w-full space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <div className="glass-card p-5 rounded-2xl border border-emerald-500/10 flex flex-col gap-4">
              <div className="flex items-center gap-4">
                <div className="w-12 h-12 rounded-full bg-emerald-500/10 border border-emerald-500/20 flex items-center justify-center font-extrabold text-amber-400 shrink-0">
                  {child.nama_santri.charAt(0)}
                </div>
                <div className="min-w-0">
                  <h3 className="text-sm font-bold text-gray-100 truncate">{child.nama_santri}</h3>
              ...
              <div className="pt-2 border-t border-emerald-950/50 flex justify-between items-center text-xs mt-auto">
            ...
            <div className="glass-card p-5 rounded-2xl border border-emerald-500/10 flex flex-col justify-between">
              ...
              <div className="pt-2 border-t border-emerald-950/50 flex justify-between items-center text-[10px] text-gray-400 mt-auto">
                ...
                {(() => {
                  ...
                  return (
                    <div className="flex items-center gap-2 mt-2">
                      ...
                      {status === "HADIR" && (
                        <span className="text-[10px] text-gray-400 font-mono">Pukul 10:06</span>
                      )}
                    </div>
                  );
                })()}
            ...
            {/* Latest Izin Pulang State Card */}
            <div className="glass-card p-5 rounded-2xl border border-emerald-500/10 flex flex-col justify-between">
              <div>
                <div className="flex justify-between items-center">
                  <span className="text-[10px] text-emerald-500/60 uppercase tracking-wider font-mono font-bold block">IZIN PULANG TERBARU</span>
                  <button
                    onClick={() => switchTab('wali-izin-pulang')}
                ...
                {(() => {
                  const latestIzin = childIzinPulangList[0];
                  if (!latestIzin) {
                    return <p className="text-xs text-gray-400 mt-2 italic">Belum ada riwayat izin pulang.</p>;
                  }
                  const isReturned = latestIzin.status === 'KEMBALI';
                  const isOverdue = !isReturned && latestIzin.status === 'DISETUJUI' && latestIzin.tanggal_kembali < todayStr;
                  return (
                    <div className="mt-2 space-y-1">
                      <span className={`inline-block px-2.5 py-0.5 rounded-full text-[10px] font-extrabold border ${
                        isOverdue ? 'bg-red-600/20 text-red-300 border-red-500/50 animate-pulse' :
                        latestIzin.status === 'KEMBALI' ? 'bg-emerald-500/10 text-emerald-400 border-emerald-500/20' :
                        latestIzin.status === 'DISETUJUI' ? 'bg-amber-500/10 text-amber-300 border-amber-500/20' :
                        latestIzin.status === 'MENUNGGU' ? 'bg-sky-500/10 text-sky-300 border-sky-500/20' :
                        'bg-red-500/10 text-red-400 border-red-500/20'
                      }`}>
                        {isOverdue ? '🚨 TERLAMBAT KEMBALI' : latestIzin.status === 'DISETUJUI' ? '🏠 SEDANG IZIN PULANG' : latestIzin.status}
                      </span>
                      <p className="text-xs text-gray-200 font-medium truncate mt-0.5">
                        {latestIzin.tanggal_keluar} s/d {latestIzin.tanggal_kembali}
                      </p>
                    </div>
                  );
                })()}
              ...
              <div className="pt-2 border-t border-emerald-950/50 flex justify-between items-center text-[10px] text-gray-400 mt-auto">
                ...
                <span className="font-bold text-amber-400">{childIzinPulangList.length} Kali</span>
              </div>
            </div>
      ...
      {/* 2. TAB IZIN PULANG SANTRI */}
      {activeTab === 'wali-izin-pulang' && (
        <div className="space-y-6">
          {/* Header Banner */}
          <div className="glass-card p-6 rounded-3xl border border-sky-500/20 bg-gradient-to-br from-emerald-950/40 via-[#03231e]/70 to-[#021814]/90 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
            <div>
              <div className="flex items-center gap-2">
                <span className="text-[10px] uppercase font-mono tracking-widest text-sky-400 bg-sky-400/10 px-2.5 py-1 rounded-full border border-sky-400/20 flex items-center gap-1.5">
                  <LucideIcon name="door-open" className="w-3.5 h-3.5" />
                  Pos Perizinan Santri
                </span>
                <span className="text-[10px] text-amber-400 bg-amber-400/10 px-2 py-0.5 rounded-full font-mono font-bold">
                  {child.nama_santri} ({child.kelas})
                </span>
              </div>
              <h2 className="text-xl font-extrabold text-white mt-2">Riwayat & Pengajuan Izin Pulang</h2>
              <p className="text-xs text-emerald-400/80 mt-1 max-w-xl">
                Pantau histori izin keluar/pulang ke rumah, status persetujuan keamanan, batas tanggal kembali, serta ajukan izin pulang baru untuk santri.
              </p>
            </div>

            <button
              onClick={() => {
                setIzinKeluarDate(todayStr);
                setIzinKembaliDate(todayStr);
                setIzinAlasan("");
                setIzinPenjemput(child.nama_wali || "Orang Tua / Wali");
                setShowIzinModal(true);
              }}
              className="px-5 py-2.5 bg-gradient-to-r from-emerald-500 to-teal-600 hover:from-emerald-400 hover:to-teal-500 text-white font-extrabold rounded-xl text-xs shadow-lg transition-all flex items-center gap-2 shrink-0 cursor-pointer active:scale-95"
            >
              <LucideIcon name="plus-circle" className="w-4 h-4" />
              <span>Buat Pengajuan Izin Pulang</span>
            </button>
          </div>

          {/* Quick Summary Badges */}
          <div className="grid grid-cols-2 sm:grid-cols-5 gap-3">
            <div className="bg-emerald-950/30 border border-emerald-500/20 rounded-2xl p-3.5 text-center">
              <span className="text-[10px] text-emerald-500/70 font-mono font-bold uppercase block">Total Izin</span>
              <strong className="text-xl font-extrabold text-white mt-0.5 block">{childIzinPulangList.length}</strong>
            </div>
            <div className="bg-amber-950/30 border border-amber-500/20 rounded-2xl p-3.5 text-center">
              <span className="text-[10px] text-amber-400/70 font-mono font-bold uppercase block">Sedang Pulang</span>
              <strong className="text-xl font-extrabold text-amber-300 mt-0.5 block">
                {childIzinPulangList.filter(i => i.status === 'DISETUJUI' && i.tanggal_kembali >= todayStr).length}
              </strong>
            </div>
            <div className="bg-red-950/40 border border-red-500/30 rounded-2xl p-3.5 text-center">
              <span className="text-[10px] text-red-400/80 font-mono font-bold uppercase block">Terlambat</span>
              <strong className="text-xl font-extrabold text-red-400 mt-0.5 block">
                {childIzinPulangList.filter(i => i.status === 'DISETUJUI' && i.tanggal_kembali < todayStr).length}
              </strong>
            </div>
            <div className="bg-sky-950/30 border border-sky-500/20 rounded-2xl p-3.5 text-center">
              <span className="text-[10px] text-sky-400/70 font-mono font-bold uppercase block">Menunggu</span>
              <strong className="text-xl font-extrabold text-sky-300 mt-0.5 block">
                {childIzinPulangList.filter(i => i.status === 'MENUNGGU').length}
              </strong>
            </div>
            <div className="bg-emerald-950/30 border border-emerald-500/20 rounded-2xl p-3.5 text-center col-span-2 sm:col-span-1">
              <span className="text-[10px] text-emerald-400/70 font-mono font-bold uppercase block">Sudah Kembali</span>
              <strong className="text-xl font-extrabold text-emerald-400 mt-0.5 block">
                {childIzinPulangList.filter(i => i.status === 'KEMBALI').length}
              </strong>
            </div>
          </div>

          {/* Table / List View Card */}
          <div className="glass-card p-5 rounded-2xl border border-emerald-500/15 space-y-4">
            
            {/* Filter Bar */}
            <div className="flex flex-col sm:flex-row items-center justify-between gap-3 border-b border-emerald-950/50 pb-3">
              <div className="flex items-center gap-2">
                <LucideIcon name="history" className="w-5 h-5 text-amber-400" />
                <h3 className="text-sm font-bold text-gray-200">Daftar Histori Izin Pulang Santri</h3>
              </div>
              ...
              <div className="flex items-center gap-2 w-full sm:w-auto flex-wrap justify-end">
                <div className="relative w-full sm:w-48">
                  <input
                    type="text"
                    value={izinSearchText}
                    onChange={(e) => setIzinSearchText(e.target.value)}
                    placeholder="Cari alasan / penjemput..."
                    className="w-full bg-emerald-950/60 border border-emerald-900/60 text-xs px-2.5 py-1.5 pl-7 rounded-lg text-emerald-300 focus:outline-none"
                  />
                  <LucideIcon name="search" className="w-3.5 h-3.5 text-emerald-500/50 absolute left-2 top-2.5" />
                </div>

                <select
                  value={izinFilterStatus}
                  onChange={(e) => setIzinFilterStatus(e.target.value as any)}
                  className="bg-emerald-950/60 border border-emerald-900/60 text-xs px-2.5 py-1.5 rounded-lg text-amber-400 focus:outline-none font-semibold"
                >
                  <option value="ALL">Semua Status ({childIzinPulangList.length})</option>
                  <option value="MENUNGGU">Menunggu Approval</option>
                  <option value="DISETUJUI">Sedang Pulang (Aktif)</option>
                  <option value="TERLAMBAT">🚨 Terlambat Kembali</option>
                  <option value="KEMBALI">Sudah Kembali Ke Pondok</option>
            ...
            <div className="space-y-3">
              {(() => {
                const filtered = childIzinPulangList.filter(item => {
                  const isReturned = item.status === 'KEMBALI';
                  const isOverdue = !isReturned && item.status === 'DISETUJUI' && item.tanggal_kembali < todayStr;

                  if (izinFilterStatus === "MENUNGGU" && item.status !== "MENUNGGU") return false;
                  if (izinFilterStatus === "DISETUJUI" && (item.status !== "DISETUJUI" || isOverdue)) return false;
                  if (izinFilterStatus === "TERLAMBAT" && !isOverdue) return false;
                  if (izinFilterStatus === "KEMBALI" && item.status !== "KEMBALI") return false;
                  if (izinFilterStatus === "DITOLAK" && item.status !== "DITOLAK") return false;

                  if (izinSearchText.trim()) {
                    const q = izinSearchText.toLowerCase();
                    const matchAlasan = item.alasan.toLowerCase().includes(q);
                    const matchPenjemput = (item.penjemput || "").toLowerCase().includes(q);
                    const matchId = item.id_izin_pulang.toLowerCase().includes(q);
                    if (!matchAlasan && !matchPenjemput && !matchId) return false;
                  }
                ...
                if (filtered.length === 0) {
                  return (
                    <div className="text-center py-12 text-gray-500 italic text-xs">
                      Tidak ada riwayat izin pulang yang sesuai dengan filter.
                    </div>
                  );
                }
                ...
                return filtered.slice().reverse().map(item => {
                  ...
                  return (
                    ...
                    >
                      <div className="flex flex-wrap justify-between items-start gap-2 border-b border-emerald-950/40 pb-2.5">
                        <div className="flex items-center gap-2">
                          <span className="text-[10px] font-mono text-emerald-400 bg-emerald-950 px-2 py-0.5 rounded border border-emerald-800">
                            {item.id_izin_pulang}
                          </span>
                          <span className="text-xs font-bold text-gray-200">
                            {item.nama_santri} ({item.kelas})
                          </span>
                        </div>
                        ...
                        <div>
                          {isOverdue ? (
                            <span className="px-3 py-1 rounded-full text-[10px] font-black bg-red-600 text-white border border-red-400 animate-pulse shadow-md">
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-3 gap-3 my-3 text-xs">
                        <div className="bg-[#021814]/60 p-2.5 rounded-lg border border-emerald-950/60">
                          <span className="text-[10px] text-emerald-500/60 uppercase font-mono font-bold block">Jadwal Tanggal Pulang:</span>
                          <span className="text-sky-400 font-bold block mt-0.5">Mulai: {item.tanggal_keluar}</span>
                          <span className={`block font-bold mt-0.5 ${isOverdue ? 'text-red-400 font-extrabold underline' : 'text-amber-400'}`}>
                            Batas Kembali: {item.tanggal_kembali}
                          </span>
                        </div>

                        <div className="bg-[#021814]/60 p-2.5 rounded-lg border border-emerald-950/60 md:col-span-2">
                          <span className="text-[10px] text-emerald-500/60 uppercase font-mono font-bold block">Alasan & Penjemput:</span>
                          <p className="text-gray-200 font-medium italic mt-0.5">"{item.alasan}"</p>
                          <span className="text-[10px] text-amber-300 block mt-1">
                            Wali / Penjemput: <strong>{item.penjemput || 'Orang Tua / Wali'}</strong>
                          </span>
                        </div>
                      </div>

                      {/* Footer Actions / Notes */}
                      <div className="flex flex-wrap justify-between items-center gap-2 pt-2 border-t border-emerald-950/40 text-[10px] text-emerald-500/60">
                        <span>
                          Otorisasi/Dicatat Oleh: <strong className="text-emerald-400">{item.dicatat_oleh || 'Pos Keamanan'}</strong> ({item.tanggal_dicatat || item.tanggal_keluar})
                        </span>

                        <div className="flex items-center gap-2">
                          {item.status === 'MENUNGGU' && (
                            <button
                              onClick={() => handleCancelIzin(item.id_izin_pulang)}
                              className="px-2.5 py-1 bg-red-950/80 hover:bg-red-900 text-red-300 border border-red-800 rounded-lg text-[10px] font-bold transition-all cursor-pointer"
                            >
                              Batalkan Pengajuan
                            </button>
                          )}
                          ...
                          {isOverdue && (
                            <a
                              href={`https://api.whatsapp.com/send?text=${encodeURIComponent(
                                `Assalamualaikum Wr. Wb. Kami dari Wali Santri *${item.nama_santri}* (${item.kelas}) mengonfirmasi terkait keterlambatan kembali tanggal *${item.tanggal_kembali}*. Mohon petunjuk dari Pos Keamanan Pondok.`
                              )}`}
                              target="_blank"
                              rel="noopener noreferrer"
                              className="px-2.5 py-1 bg-teal-800 hover:bg-teal-700 text-white rounded-lg text-[10px] font-bold transition-all flex items-center gap-1 cursor-pointer"
                            >
                              <LucideIcon name="phone-call" className="w-3 h-3 text-emerald-400" />
                              <span>Hubungi Keamanan (WA)</span>
                            </a>
                          )}
                        </div>
                      </div>
      ...
      {/* Modal Pengajuan Izin Pulang Baru */}
      {showIzinModal && (
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-[9999] flex items-center justify-center p-4 animate-fade-in">
          <div className="glass-card max-w-md w-full p-6 rounded-3xl border border-emerald-500/30 shadow-2xl space-y-4">
            <div className="flex justify-between items-center border-b border-emerald-950/60 pb-3">
              <div className="flex items-center gap-2 text-amber-400">
                <LucideIcon name="door-open" className="w-5 h-5" />
                <h3 className="text-sm font-bold text-white">Form Pengajuan Izin Pulang</h3>
              </div>
              <button
                onClick={() => setShowIzinModal(false)}
                className="text-gray-400 hover:text-white text-xs p-1 rounded-lg bg-emerald-950 border border-emerald-900 cursor-pointer"
              >
                ✕
              </button>
            </div>

            <form onSubmit={handleApplyIzinPulang} className="space-y-3 text-xs">
              <div className="bg-emerald-950/40 p-3 rounded-xl border border-emerald-900/60 text-[11px]">
                <span className="text-gray-400 block">Nama Santri Pemohon:</span>
                <strong className="text-amber-400 text-xs font-bold block mt-0.5">{child.nama_santri} ({child.kelas})</strong>
              </div>

              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-[10px] font-semibold text-emerald-400 uppercase mb-1">Tanggal Keluar (Pulang)</label>
                  <input
                    type="date"
                    required
                    value={izinKeluarDate}
                    onChange={(e) => setIzinKeluarDate(e.target.value)}
                    className="w-full bg-emerald-950/80 border border-emerald-900 text-gray-200 px-3 py-2 rounded-xl focus:outline-none focus:border-amber-500 font-mono text-xs"
                  />
                </div>

                <div>
                  <label className="block text-[10px] font-semibold text-amber-400 uppercase mb-1">Batas Tanggal Kembali</label>
                  <input
                    type="date"
                    required
                    value={izinKembaliDate}
                    onChange={(e) => setIzinKembaliDate(e.target.value)}
                    className="w-full bg-emerald-950/80 border border-emerald-900 text-gray-200 px-3 py-2 rounded-xl focus:outline-none focus:border-amber-500 font-mono text-xs"
                  />
                </div>
              </div>
              ...
              <div>
                <label className="block text-[10px] font-semibold text-emerald-400 uppercase mb-1">Wali / Penjemput</label>
                <input
                  type="text"
                  required
                  value={izinPenjemput}
                  onChange={(e) => setIzinPenjemput(e.target.value)}
                  placeholder="Nama orang tua/wali/penjemput"
                  className="w-full bg-emerald-950/80 border border-emerald-900 text-gray-200 px-3 py-2 rounded-xl focus:outline-none focus:border-amber-500 text-xs"
                />
              </div>

              <div>
                <label className="block text-[10px] font-semibold text-emerald-400 uppercase mb-1">Alasan Izin Pulang</label>
                <textarea
                  rows={3}
                  required
                  value={izinAlasan}
                  onChange={(e) => setIzinAlasan(e.target.value)}
                  placeholder="Contoh: Acara keluarga pernikahan/khitanan, pengobatan medis, dll."
                  className="w-full bg-emerald-950/80 border border-emerald-900 text-gray-200 px-3 py-2 rounded-xl focus:outline-none focus:border-amber-500 text-xs"
                ></textarea>
              </div>

              <div className="pt-3 border-t border-emerald-950/60 flex justify-end gap-2">
                <button
                  type="button"
                  onClick={() => setShowIzinModal(false)}
                  className="px-4 py-2 bg-emerald-950/60 hover:bg-emerald-900 text-emerald-400 border border-emerald-900 rounded-xl font-semibold cursor-pointer"
                >
                  Batal
                </button>
                <button
                  ...
                  className="px-4 py-2 bg-gradient-to-r from-emerald-600 to-emerald-500 hover:from-emerald-500 hover:to-emerald-400 text-white font-bold rounded-xl shadow transition-all cursor-pointer"
                >
                  {isSubmittingIzin ? "Mengirim..." : "Kirim Permohonan"}
                </button>
              </div>
            </form>
          </div>

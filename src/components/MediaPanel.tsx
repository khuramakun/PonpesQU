  ...
  const handleImageFileChange = async (e: React.ChangeEvent<HTMLInputElement>, setter: (val: string) => void) => {
    ...
    if (file) {
      try {
        ...
        setter(compressed);
        showToast("✓ Gambar berhasil dimuat & dioptimasi!", "success");
      } catch (err) {
                    ...
                    <div className="flex gap-1.5 shrink-0">
                      <button 
                        ...
                        onClick={() => {
                          ...
                          showToast("✓ Link berita disalin ke papan klip!", "success");
                        }}
                        className="p-1.5 bg-emerald-900/30 hover:bg-emerald-900/60 text-sky-400 rounded-lg border border-emerald-900/40 transition-colors cursor-pointer"
                        title="Salin Link Direct Berita"
                      >
                        <LucideIcon name="share-2" className="w-3.5 h-3.5" />
                      </button>
                      <button 
                        type="button"
                        onClick={() => openEditNewsModal(item)}
                        className="p-1.5 bg-emerald-900/30 hover:bg-emerald-900/60 text-amber-400 rounded-lg border border-emerald-900/40 transition-colors cursor-pointer"
                        title="Edit Berita"
                      >

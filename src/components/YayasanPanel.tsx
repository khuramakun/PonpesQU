...
export function YayasanPanel({
  ...
  const [kasPrintEndDate, setKasPrintEndDate] = useState(() => new Date().toISOString().slice(0, 10));
  ...
  const generateUsername = () => {
    if (!santriForm.nama_santri) {
      showToast("Tulis nama santri terlebih dahulu!", "error");
      return;
    }
    const cleanName = santriForm.nama_santri.trim().toLowerCase().split(' ')[0].replace(/[^a-z0-9]/g, '');
  ...
  const handleSantriPhoto = async (event: React.ChangeEvent<HTMLInputElement>) => {
    const file = event.target.files?.[0];
    if (!file) return;
    try {
      const compressed = await compressImageFile(file, 250, 0.6);
      setSantriForm(prev => ({ ...prev, foto_profil: compressed }));
      showToast("✓ Foto Profil berhasil diunggah & dioptimasi!", "info");
    } catch (e) {
  ...
  const saveImportedSantri = async () => {
    ...
    validRows.forEach(row => {
      // Check if existing
      const existingIdx = updatedSantri.findIndex(s => s.barcode === row.barcode || s.nama_santri.toLowerCase().trim() === s.nama_santri.toLowerCase().trim());
      
      const defaultUser = row.nama_santri 
        ? row.nama_santri.trim().toLowerCase().split(' ')[0].replace(/[^a-z0-9]/g, '') + Math.floor(100 + Math.random() * 900) 
        : "santri" + Math.floor(1000 + Math.random() * 9000);
      ...
      const payload: Santri = {
        ...
        username: row.username || (existingIdx > -1 ? updatedSantri[existingIdx].username : undefined) || defaultUser,
        pass: row.pass || (existingIdx > -1 ? updatedSantri[existingIdx].pass : undefined) || defaultPass
      };
  ...
  const handleTeacherPhoto = async (event: React.ChangeEvent<HTMLInputElement>) => {
    const file = event.target.files?.[0];
    if (!file) return;
    try {
      const compressed = await compressImageFile(file, 250, 0.6);
      setTeacherForm(prev => ({ ...prev, foto_profil: compressed }));
      showToast("✓ Foto pengajar berhasil diunggah & dioptimasi!", "info");
    } catch (e) {
      showToast("Gagal memuat foto pengajar", "error");
    }
  };
              ...
              {filteredSantri.map(s => (
                <tr key={s.id_santri} className="hover:bg-emerald-950/10 border-b border-emerald-950/10 transition-colors">
                  ...
                  <td className="py-3">
                    ...
                    {/* Kredensial Login */}
                    <div className="text-[9px] text-amber-400 font-mono mt-1 flex items-center gap-1 bg-amber-500/5 px-2 py-0.5 rounded border border-amber-500/10 w-fit">
                      <span>🔑</span>
                      <span>User: <span className="font-bold">{s.username || '-'}</span></span>
                      <span className="text-emerald-500/30">|</span>
                      <span>PW: <span className="font-bold">{s.pass || '-'}</span></span>
                    </div>
                  </td>
          ...
          <dialog open className="backdrop:bg-[#02110e]/80 fixed inset-0 z-50 bg-transparent focus:outline-none p-4 w-full max-w-md flex items-center justify-center">
            <div className="glass-card p-6 rounded-2xl border border-amber-500/30 flex flex-col gap-4 w-full">
              ...
              <form onSubmit={saveSantri} className="space-y-3 text-xs text-gray-300">
                ...
                {/* Kredensial Login Santri */}
                <div className="p-3.5 bg-emerald-950/40 rounded-xl border border-emerald-900/50 space-y-3.5">
                  <span className="text-[10px] font-extrabold text-amber-500 uppercase tracking-wider block">Kredensial Login Santri</span>
                  <div className="grid grid-cols-2 gap-3">
                    <div>
                      <label className="block text-emerald-500/60 mb-1">Username (Sandi)</label>
                      <div className="flex gap-1.5">
                        <input 
                          type="text" 
                          required
                          value={santriForm.username}
                          onChange={(e) => setSantriForm({ ...santriForm, username: e.target.value.toLowerCase().replace(/\s+/g, '') })}
                        ...
                        <button
                          ...
                          className="px-2 bg-emerald-900 border border-emerald-800 text-amber-400 rounded-xl hover:bg-emerald-800 transition-all flex items-center justify-center cursor-pointer"
                        >
                          <LucideIcon name="wand" className="w-3.5 h-3.5" />
                        </button>
                      </div>
                    </div>
                    <div>
                      <label className="block text-emerald-500/60 mb-1">Kata Sandi (PW)</label>
                      <input 
                        type="text" 
                        required
                        value={santriForm.pass}
                        onChange={(e) => setSantriForm({ ...santriForm, pass: e.target.value })}
